...
{
	CreateConVar("sm_anticamp_version", PLUGIN_VERSION, "Anti-camp Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	...
	g_CvarLowHealth = CreateConVar("sm_anticamp_low_health_level", "0", "Set to the health level below which camping is OK");
...
public BeamRing(client, color[4]) {
	Player(client).beacon_ping({128, 128, 128, 255});
}