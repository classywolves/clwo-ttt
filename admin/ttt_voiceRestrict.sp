...
public void OnClientSpeakingEx(int client) {
    if (voiceRestrictEnabled && !isPlayerSpeaking[client]) {
        ...
        if (isPlayerSpeaking[client] || player.Access(RANK_INFORMER, false)) { return; }
...
public void OnClientSpeakingEnd(int client) {
    if (voiceRestrictEnabled && isPlayerSpeaking[client]) {
        ...
        if (timeSpoken > 240) {
            player.Mute(240, "You have been muted for excessive mic usage for the next 2 minutes.")
        }
    }
}
...
public Action Command_VoiceRestrict(int client, int args) {
    if (args < 2) {
        ...
        char param[5];
        GetCmdArg(1, param, 5);
        if (StrCompare(param, "true", false) || param[0] == '1') {
            voiceRestrictEnabled = true;
        }
        else if (StrCompare(param, "false", false) || param[0] == '0') {
    ...
    CPrintToChatAll("{purple}[TTT] {yellow}Voice Restrict has now been %s.", voiceRestrictEnabled ? "{green}Enabled" : "{red}Disabled");
