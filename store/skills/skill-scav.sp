...
#define SCAV_PRICE 500
#define SCAV_STEP 1.1
#define SCAV_LEVEL 2
#define SCAV_SORT 0

public Plugin myinfo =
{
    name = "Skill Scavenger",
    author = "Popey & c0rp3n",
...
{
    Store_RegisterSkill(SCAV_ID, SCAV_NAME, SCAV_DESCRIPTION, SCAV_PRICE, SCAV_STEP, SCAV_LEVEL, SCAV_SORT);
}
...
public void Store_OnReady()
{
    LoopValidClients(i)
    {
        g_playerData[i].level = Store_GetSkill(i, SCAV_ID);
    }
}
...
public void OnClientPutInServer(int client)
{
    if (Store_IsReady())
    {
        g_playerData[client].level = Store_GetSkill(client, SCAV_ID);
    }
}
...
public void TTT_OnBodyFound(int client, int victim, int entityref, bool silentID)
{
    if (g_playerData[client].level)
    {
        ...
        if (IsValidEdict(weapon))
        {
            ...
            int capacity = GetWeaponMagCapacity(weaponName);
            ...
            CPrintToChat(client, "{default}[TTT] > You found {orange}%d {default}round on {yellow}%N's {default}body thanks to {yellow}Scavenger.", scavenged, victim);
        }
    }
}
