...
{
    name = "TTT Mapcooser",
    author = "c0rp3n & AlliedModders LLC",
    description = "Automated Map Voting for CLWO TTT.",
    version = "1.0.0",
    url = ""
};
...
public void OnPluginStart()
{
    ...
    PrintToServer("[MAP] Loaded successfully");
}
...
public void RegisterConVars()
{
    ...
    AutoExecConfig_SetFile("ttt_mapchooser", "ttt");

    g_Cvar_EndOfMapVote = AutoExecConfig_CreateConVar("ttt_mapvote_endvote", "1", "Specifies if MapChooser should run an end of map vote", _, true, 0.0, true, 1.0);
    g_Cvar_MaxRounds = AutoExecConfig_CreateConVar("ttt_mapvote_maxrounds", "20", "Specifies how many rounds should start before a vote shall initiate.", _, true, 1.0);
    g_Cvar_ExtendRoundStep = AutoExecConfig_CreateConVar("ttt_extendmap_roundstep", "5", "Specifies how many more rounds each extension makes.", _, true, 1.0);
    g_Cvar_ExcludeMaps = AutoExecConfig_CreateConVar("ttt_mapvote_exclude", "5", "Specifies how many past maps to exclude from the vote.", _, true, 0.0);
    g_Cvar_IncludeMaps = AutoExecConfig_CreateConVar("ttt_mapvote_include", "5", "Specifies how many maps to include in the vote.", _, true, 2.0, true, 6.0);
    g_Cvar_NoVoteMode = AutoExecConfig_CreateConVar("ttt_mapvote_novote", "1", "Specifies whether or not MapChooser should pick a map if no votes are received.", _, true, 0.0, true, 1.0);
    g_Cvar_Extend = AutoExecConfig_CreateConVar("ttt_mapvote_extend", "0", "Number of extensions allowed each map.", _, true, 0.0);
    ...
    g_Cvar_VoteDuration = AutoExecConfig_CreateConVar("ttt_mapvote_voteduration", "20", "Specifies how long the mapvote should be available for.", _, true, 5.0);
    g_Cvar_RunOff = AutoExecConfig_CreateConVar("ttt_mapvote_runoff", "0", "Hold run of votes if winning choice is less than a certain margin.", _, true, 0.0, true, 1.0);
    g_Cvar_RunOffPercent = AutoExecConfig_CreateConVar("ttt_mapvote_runoffpercent", "50", "If winning choice has less than this percent of votes, hold a runoff.", _, true, 0.0, true, 100.0);
...
public Action TTT_OnRoundStart_Pre()
{
    ...
    {
        InitiateVote(MapChange_Instant, null);
...
void InitiateVote(MapChange when, ArrayList inputlist=null)
{
    ...
    */
    ...
    CPrintToChatAll("{purple}[TTT] {yellow}%t", "Nextmap Voting Started");
}
...
public void Handler_VoteFinishedGeneric(Menu menu,  int num_votes,  int num_clients,  const int[][] client_info,   int num_items,   const int[][] item_info)
{
    ...
    if (strcmp(map, VOTE_EXTEND, false) == 0)
    {
        ...
        CPrintToChatAll("{purple}[TTT] {yellow}%t", "Current Map Extended", RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
        ...
        DataPack data;
        g_VoteTimer = CreateDataTimer(5.0, Timer_StartMapVote, data, TIMER_FLAG_NO_MAPCHANGE);
    ...
    {
        CPrintToChatAll("{purple}[TTT] {yellow}%t", "Current Map Stays", RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
    ...
    else
    {
        ...
        CPrintToChatAll("{purple}[TTT] {yellow}%t", "Nextmap Voting Finished", displayName, RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
...
public void Handler_MapVoteFinished(Menu menu, int num_votes,  int num_clients, const int[][] client_info,  int num_items, const int[][] item_info)
{
    if (g_Cvar_RunOff.BoolValue && num_items > 1)
    {
        ...
        if (winningvotes < required)
        {
            ...
            CPrintToChatAll("{purple}[TTT] {yellow}%t", "Starting Runoff", g_Cvar_RunOffPercent.FloatValue, info1, map1percent, info2, map2percent);
